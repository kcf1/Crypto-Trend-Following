...
from utils import align_idx,winsorize
...
for i,symbol in enumerate(symbols):
    ...
    fc = .30 / 360
    poss = dict()
    df = read_mtbars(symbol,limit=24*360*10)
    df['volume'] = df['tick_volume']
    ret = np.log(df['close']).diff().shift(-1)
    vol = rogers_satchell_volatility(df,window=24*12) * 12**0.5
    ...
    #try:
        ...
        y = np.log(df['close']).diff(12).shift(-12) / vol
        x = pd.DataFrame(poss)
        x,y = align_idx(x,y)
        x1,x2 = train_test_split(x,test_size=0.5,shuffle=False)
        y1,y2 = y.reindex(x1.index),y.reindex(x2.index)
        #y1 = winsorize(y1,.05,.05)

        #ols = OLS(y1,add_constant(x1)).fit()
        #print(ols.summary())
        #yh = ols.predict(add_constant(x2))
        tscv = TimeSeriesSplit(n_splits=10,max_train_size=24*720)
        ols = RidgeCV(fit_intercept=False,cv=tscv)
        #ols = RandomForestRegressor(n_estimators=500,max_depth=3,random_state=42,n_jobs=-1)
        ols.fit(x1,y1)
        print(ols.score(x1,y1),ols.score(x2,y2))
        yh = pd.Series(ols.predict(x2),index=x2.index)
        scatter(yh,y2,ax=axes[0,i])

        exp_ret = yh * vol.reindex(yh.index)
        direction = pd.Series(np.where(exp_ret.abs() > 0.0030, yh, 0), index=exp_ret.index)
        pos = direction * 0.30 / 10 / vol
        funding = pd.Series(np.where(pos.index.hour == 22, 1, 0),pos.index)
        pos = pos.mask(funding==1,0)
        pnl = pos * ret - pos.diff().abs() * tc - pos.abs() * funding * fc
        pnl_curve(pnl.reindex(x2.index),ax=axes[1,i])

        pos = x2.sum(axis=1)
        pnl = pos * ret - pos.diff().abs() * tc - pos.abs() * funding * fc
        pnl_curve(pnl.reindex(x2.index),ax=axes[2,i])
